...
function makeThreadDistributionBar(
  h: number,
  g: number,
  w: number,
  total: number,
  ...
  return "█".repeat(hLen) + "▒".repeat(gLen) + "░".repeat(wLen);
}

export function drawShotgunDashboard(
  ns: NS,
  data: ShotgunDashboardData,
): void {
  ns.clearLog();
  ...
  const ramPercent =
    data.ramTotal > 0 ? (data.ramUsed / data.ramTotal) * 100 : 0;
  const ramBar = makeProgressBar(ramPercent / 100, 20);

  const { hack, grow, weaken, total } = data.activeThreads;
  const threadDistBar = makeThreadDistributionBar(
    hack,
    grow,
    weaken,
    total,
  ...
  ns.print(
    `RAM Pool:        ${ns.format.ram(data.ramUsed)} / ${ns.format.ram(data.ramTotal)}`,
  );
  ns.print(`------------------------------------------------------------`);
  ns.print(`LAUFENDE THREADS (GESAMT: ${ns.format.number(total)}):`);
  ns.print(`Verteilung:      [${threadDistBar}]`);
  ns.print(
    `  🪓 Hack:   ${ns.format.number(hack).padStart(7)} threads (${total > 0 ? ((hack / total) * 100).toFixed(1) : 0}%)`,
  );
  ns.print(
    `  🌱 Grow:   ${ns.format.number(grow).padStart(7)} threads (${total > 0 ? ((grow / total) * 100).toFixed(1) : 0}%)`,
  );
  ns.print(
    `  🛡️ Weaken: ${ns.format.number(weaken).padStart(7)} threads (${total > 0 ? ((weaken / total) * 100).toFixed(1) : 0}%)`,
  );
  ns.print(`------------------------------------------------------------`);
