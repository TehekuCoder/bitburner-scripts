...
export async function main(ns: NS): Promise<void> {
  ...
  while (true) {
    ...
    // Starte Dispatcher, falls bereit
    if (
      homeMax >= 256 &&
      ...
      isDispatcherRunning = true; // Flag sofort setzen, damit der Kernel nicht auf den nächsten Game-Tick wartet
    }
    ...
    // ======================================================================
    // --- 🧹 CLEANUP & 📡 WORKER DEPLOYMENT (VEREINFACHT) ---
    // ======================================================================
    for (const node of allNodes) {
      if (!ns.hasRootAccess(node)) continue;

      // 1. WENN DISPATCHER/BATCHER AKTIV IST:
      // Der Dispatcher hat seine eigene Bereinigungs-Logik.
      // Der Kernel darf hier NICHT blind killen, sonst entsteht ein Ping-Pong-Effekt!
      if (isDispatcherRunning) {
        // Wir übergeben die volle Kontrolle über den Server-RAM an den Dispatcher.
        continue;
      }
