...
export async function main(ns: NS): Promise<void> {
  ns.disableLog("ALL");
  // ns.tail(); // Optional: Aktivieren, wenn sich das Fenster beim Start direkt öffnen soll
    ...
    for (let i = 0; i < numSleeves; i++) {
        ...
        if (purchasableAugs.length > 0) {
          ...
          if (ns.getPlayer().money > cheapestAug.cost * BUDGET_MULTIPLIER) {
            if (ns.sleeve.purchaseSleeveAug(i, cheapestAug.name)) {
              ns.print(`🛒 [Sleeve ${i}] Augment erworben: ${cheapestAug.name}`);
            }
          }
        }
      } catch {
      ...
      if (factionsNeedingRep.length > 0) {
        ...
        if (currentTask?.type === "FACTION") {
          const currentFaction = (currentTask as any).factionName as FactionName;
        ...
        if (!targetFaction) {
          ...
          if (availableFactions.length > 0) {
            if (i === 0 && currentState?.targetFaction && availableFactions.includes(currentState.targetFaction as FactionName)) {
        ...
        if (targetFaction) {
          if (currentTask?.type === "FACTION" && (currentTask as any).factionName === targetFaction) {
            continue; 
          }

          const workTypes = ["hacking", "field", "security"] as unknown as FactionWorkType[];
          let assigned = false;
