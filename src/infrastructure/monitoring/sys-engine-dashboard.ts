...
import { EngineDashboardData, drawEngineDashboard } from "/ui/engine-ui";
  ...
  while (true) {
    ...
    for (const s of servers) {
      ...
      for (const proc of ns.ps(s)) {
        if (
          proc.filename === PATHS.services.payloads.hack ||
          proc.filename.endsWith(PATHS.services.payloads.hack)
        ) {
          totalHack += proc.threads;
        } else if (
          proc.filename === PATHS.services.payloads.grow ||
          proc.filename.endsWith(PATHS.services.payloads.grow)
        ) {
          totalGrow += proc.threads;
        } else if (
          proc.filename === PATHS.services.payloads.weaken ||
          proc.filename.endsWith(PATHS.services.payloads.weaken)
        ) {
