...
export function evaluateHackingStrategy(ns: NS): StrategyRecommendation {
  ...
  const desyncRisk = mults.ServerStartingSecurity / (mults.HackingSpeedMultiplier * mults.ServerWeakenRate);

  // 0. XP_GRIND Check: Early-Game (< 30 Skill) oder Null-Ertrag BitNodes
  if (playerSkill < 30 || yieldFactor === 0) {
    return {
      strategy: "XP_GRIND",
      reason: playerSkill < 30
  ...
  if (!hasFormulas && effectiveRam < 1024) {
    return {
      strategy: "PROTO_BATCH",
      reason: "Keine Formulas.exe und moderater RAM. Sequentieller Proto-Batch ist am stabilsten.",
      yieldFactor,
      effectiveRam,
    };
  }
  ...
  if (!hasFormulas) {
    return {
      ...
      reason: "Viel RAM vorhanden, aber keine Formulas.exe. Shotgun/Burst HWGW bietet optimalen Durchsatz.",
      yieldFactor,
      effectiveRam,
    };
  }
  ...
  if (desyncRisk <= 2.0 && effectiveRam >= 1024) {
    return {
      strategy: "JIT_HWGW",
      reason: "High Yield, Formulas.exe vorhanden und hohes RAM-Budget. Maximale JIT-Pipeline Effizienz.",
      yieldFactor,
      effectiveRam,
    };
  }
  ...
  return {
    strategy: "SHOTGUN_HWGW",
    reason: "Gute Rahmenbedingungen, aber erhöhtes Desync-Risiko durch Server-Sicherheit.",
    yieldFactor,
    effectiveRam,
  };
}