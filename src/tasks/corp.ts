...
export async function main(ns: NS): Promise<void> {
  ...
  while (true) {
    ...
    const targetCompany =
      ((state as any)?.targetCompany as CompanyName) || undefined;
    ...
    const isAlreadyWorkingHere =
      ...
      currentWork.companyName === targetCompany;

    if (!isAlreadyWorkingHere) {
      ns.print(
        `[WORK] Starte Arbeit bei ${targetCompany} als ${currentJobTitle}...`,
      );
      const success = sing.workForCompany(targetCompany, false);

      if (!success) {
        ns.print(
          `[WARN] Konnte Arbeit nicht starten. Fehlen Qualifikationen für ${jobField}?`,
        );
      }
    }
